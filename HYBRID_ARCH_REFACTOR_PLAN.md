...
- [x] **9F-8** — DONE (eval job 1895). Removed leftover `--norm-topology pre_rms` from `eval_stage0_lm.sh:85` (first attempt, job 1894, gave garbage PPL 141.28 because eval built a pre_rms model and dropped the 20 trained HybridNorm weights). Clean re-run: `Norm topology from hybrid_70m_v2.yaml: hybrid`, `Weights loaded successfully (exact match)` (0 missing/0 unexpected), **PPL=20.38**, BPB=4.35. Bug fix VERIFIED. Gate 13.76 FAIL by 48%. **Key finding:** 20.38 == job 1401 (HybridNorm inactive) → HybridNorm is PPL-neutral at this scale; the gate miss is 100% **undertraining** (50K-from-scratch vs baseline ~117K). No arch/config debt remains.

### Phase 9-EXT — extended Stage 0 retrain to 120K steps (~30-36h A100) ⏳ NEXT
The undertraining fix. Job 1884's val/perplexity was still descending at 50K; baseline 13.10 took ~117K cumulative steps.
- [ ] **9EXT-1** — `scripts/train_stage0_arch_v2_ext.sh` (NEW): `model=hybrid_70m_v2`, `trainer.max_steps=120000`, `model.warmup_steps=1000` (WSD auto-reshapes: warmup 1000 / stable ~102200 / decay 16800 from step ~103200), `val_check_interval=2000`, `--time=36:00:00`, fresh `output_dir=./outputs/phase9_stage0_arch_v2_ext`. Config sanity-checked via `--cfg job` (norm_topology=hybrid, max_steps=120000 from `cfg.trainer.max_steps` at `train_stage0_distill.py:481`).
- [ ] **9EXT-2** — `sbatch` on willi. NOTE: resume-on-requeue NOT wired (no `ckpt_path` in `trainer.fit`) — must fit one walltime block; if partition caps below ~36h, wire `ckpt_path` resume first.
- [ ] **9EXT-3** — Extract final ckpt → `eval_stage0_lm.sh` (now reads norm_topology from yaml; works unchanged). Gate PPL ≤ 13.76. If still missing but PPL materially < 20.38 and curve still descending, extend further; if plateaued well above 13.76, investigate KD α / LR.
