...
>
> **Current phase: Phase 11 — Report-generation evaluation, full-data arm. Phase 8 (fetch+pack) and the full-data Phase 10E training run (job 2491338, 191,462 pairs) are DONE (2026-08-28). Full n=1433 eval (11C full, jobs 2491600/2491687) REVERSES the n=10 preliminary read: generator BEATS the retrieval-NN floor on all 3 metrics (rouge_l 0.2075 vs 0.1881; bleu_1 0.2706 vs 0.2605; bleu_4 0.0707 vs 0.0465). Caveat: 1055/1433 (73.6%) generated reports fall into one of 184 exact-duplicate template clusters — real signal but still heavily templated. 11B (CheXbert F1): harness built, `f1chexbert` package installed, and THREE separate bugs in that 2023-era package found and worked around by 2026-08-29 — (1) wrong assumed API, fixed; (2) `HF_HUB_OFFLINE=1` blocking its first-run download, fixed via env override; (3) its own `download_model()` swallows a `force_filename`-removed exception from `hf_hub_download`, leaving `chexbert.pth` never written — fixed by manually staging the file; (4) its `tokenize()` calls `encode_plus`, removed in `transformers>=5.0` — fixed not by patching the package but by building a fully ISOLATED venv (`scripts/setup_chexbert_venv_h100.sh`, `transformers<5`) that scores plain `hyps.txt`/`refs.txt` files via a new decoupled `scripts/score_chexbert_standalone.py`, dumped from the main pipeline by a new `--dump-dir` flag on `evaluate_report_generation.py`. See the 11B-infra checkbox below for full detail. NEXT ACTION: `sbatch scripts/setup_chexbert_venv_h100.sh` (one-time), rerun the n=1433 checkpoint inspection with `DUMP_DIR=...` to dump hyps/refs, then `DUMP_DIR=... sbatch scripts/score_chexbert_h100.sh` — the actual integration test. Also open, independent: 11D (official test split), 11E (qualitative appendix).**
