...
- [x] **15C-4** — **NOT RUN, BY THE PRE-REGISTERED RULE (2026-09-16).** No λ won the validate selection, so there is nothing to replicate across seeds; running seeds anyway, or probing further doses, would be exactly the iterate-to-target behaviour this item was written to forbid. The ~7 GPU-hours are returned to the budget. Original spec: **3 seeds of the winning λ**, judged against 15B-3's seed band and 15B-5's per-label CIs. **Pre-registered decision rule, declared before the runs:** 15C is a **success** only if CheXbert-14-macro improves by more than the seed SD from 15B-3 **and** no CheXbert micro metric regresses below the retrieval floor (the precise failure that killed 13F at both doses). Anything else is reported as a negative or intermediate result.
