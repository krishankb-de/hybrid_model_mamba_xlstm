...
#### 15B — Statistical rigor for the generation table 🟡 IN PROGRESS — 15B-1/2/3/5 done; 15B-4 CheXbert half computed 2026-09-15 (🔴 Phase 14's CheXbert wins do NOT replicate across seeds), text metrics + per-seed bootstraps pending

- [x] **15B-1** — **DONE 2026-09-13 (re-score job 2542358, bootstrap job 2542384; `analysis/bootstrap_hybrid_vs_retrieval_floor.md`).** n=2663, 1000 paired resamples. **13D wins 7 with CI excluding zero** — ROUGE-L +0.0263 [+0.0228,+0.0293], BLEU-1 +0.0097, BLEU-4 +0.0212 [+0.0181,+0.0242], CheXbert-14-micro +0.0440 [+0.0321,+0.0561], CheXbert-5-micro +0.0665 [+0.0511,+0.0827], CheXbert-5-macro +0.0203 [+0.0039,+0.0380], exact-match-5 +0.0428. **The floor wins 1: CheXbert-14-macro −0.0214 [−0.0340,−0.0081].** exact-match-14 ties (lower bound exactly +0.0000). **The macro deficit is now interval-backed, not a point estimate** — it confirms supervisor item 2 with a measurement. Original spec: **CIs on generator-vs-retrieval-floor.** This is the literal "Table 3 reports none" complaint. **Zero new code** — `scripts/bootstrap_compare.py` already takes any two hyp dumps. Blocker confirmed 2026-09-13: `results/retrieval_floor_test_split/` has `hyps.txt`/`refs.txt`/`chexbert_metrics.json` but **no `chexbert_labels.json`** (it was scored 2026-08-30, before `--dump-labels` existed), so re-score it once (~15 min) to get the label matrices, then bootstrap. **⚠ 2026-09-15 — the 14-micro margin over the floor is seed-42-dependent:** at seeds 43/44 the hybrid clears the floor's 0.4296 by only +0.0084 / +0.0028 (Transformer s44 sits *below* it, −0.0011); the macro deficit replicates at every seed (0.2572/0.2609 vs 0.3014). Per-seed bootstraps vs the floor submitted to settle it.
...
- [ ] **15B-4** — **Report mean ± SD per metric per arm, plus the hybrid−Transformer difference paired by seed.** The question a viva will actually ask: *is the +0.0146 CheXbert-14-micro win larger than seed noise?* **Pre-registered honesty constraint: 3 seeds licenses "positive in k/3 seeds, range [x, y]" and a mean ± SD. It does NOT license a p-value** (2 df), and no t-test will be reported. Two distinct uncertainty sources must be kept visibly separate in the writeup: bootstrap CI = *sampling* uncertainty over studies; seed SD = *training* uncertainty over runs. They are not interchangeable and neither subsumes the other.
  **PARTIAL 2026-09-15 — CheXbert half (seed-42 values from `h100_scaling_state.json` 14A block; seeds 43/44 from `chexbert_metrics.json`).** n=2663, beam 3, k=32, all from `last.ckpt`. `accuracy` = **5-label** exact match (f1chexbert's convention; the 14-label variant comes from the bootstrap).

  | metric | hybrid 42 / 43 / 44 | mean ± SD | Transformer 42 / 43 / 44 | mean ± SD | hybrid − Transformer, paired | mean ± SD | hybrid ahead |
  |---|---|---|---|---|---|---|---|
  | CheXbert-14-micro | .4736 / .4380 / .4324 | .4480 ± .0223 | .4590 / .4455 / .4285 | .4443 ± .0153 | +.0146 / −.0075 / +.0040 | +.0037 ± .0111 | 2/3 |
  | CheXbert-14-macro | .2800 / .2572 / .2609 | .2660 ± .0122 | .2774 / .2730 / .2572 | .2692 ± .0106 | +.0026 / −.0158 / +.0037 | −.0032 ± .0109 | 2/3 |
  | CheXbert-5-micro | .5522 / .4810 / .4928 | .5087 ± .0382 | .5249 / .5049 / .4797 | .5032 ± .0227 | +.0273 / −.0240 / +.0131 | +.0055 ± .0265 | 2/3 |
  | CheXbert-5-macro | .4487 / .3945 / .4146 | .4193 ± .0274 | .4319 / .4198 / .3993 | .4170 ± .0165 | +.0168 / −.0253 / +.0154 | +.0023 ± .0239 | 2/3 |
  | exact-match-5 | .2163 / .2144 / .2182 | .2163 ± .0019 | .2306 / .2234 / .2193 | .2244 ± .0057 | −.0143 / −.0090 / −.0011 | −.0081 ± .0066 | **0/3** |
  | example-F1 (`samples avg`) | ? / .3726 / .3615 | — | ? / .3835 / .3664 | — | ? / −.0109 / −.0049 | — | 0/2 |

  **What this licenses:**
  1. **🔴 Phase 14's "hybrid significantly wins 3 CheXbert F1 metrics" does not survive seed replication.** Every CheXbert F1 difference changes sign across seeds; at seed 43 the Transformer leads on all of them, and every paired mean is well inside one paired SD. Phase 14's CIs were real but measured *sampling* uncertainty at one training draw. Honest claim: **at matched parameters and k=32 the two architectures are indistinguishable on CheXbert F1 across 3 seeds.** The one same-sign result is exact-match-5, Transformer ahead 3/3.
  2. **Seed 42 is the high draw for both architectures** (hybrid 14-micro +.036/+.041 over seeds 43/44). Consistent with selection: 13D was chosen as the best 13-series arm, and the same-seed k=8/k=64 hybrids (0.4481/0.4488) sit with seeds 43/44, so **Phase 14's "k=32 is a significant, bracketed optimum" is likely the same draw** — flag, do not re-run.
  3. **Zero-F1 rare labels are not hybrid-specific:** Pleural Other = 0 in 3 of 4 new arms (both architectures); Transformer s44 also zeroes Pneumothorax. Supports 15C's framing as a rarity problem.
  4. Headline numbers become **mean ± SD**, not 13D: hybrid 14-micro 0.448 ± 0.022, 14-macro 0.266 ± 0.012.

  **Still open for 15B-4:** ROUGE-L / BLEU-1 / BLEU-4 / exact-match-14 for seeds 43/44 (the inspect logs print only per-sample ROUGE; aggregates come from the per-seed bootstraps), seed-42 example-F1 (`samples avg` in both seed-42 `chexbert_metrics.json`), and the 13D re-decode `cmp`.
