...
- [ ] **7D-verify** — **Still open.** Re-verify via Slurm (the login node refuses all execution — see 7E):
      ...
      ```
      (`--account=aisc --partition=aisc-batch --qos=aisc` is baked into the script's `#SBATCH` header — see 7E — so nothing extra needs passing at submit time. Also do NOT add a manual `--time=...` override: the script's own default (1 day) is generous; the one prior attempt that used a borrowed `--time=00:10:00` timed out mid-download.) Confirm it downloads the ~150 MB of small files cleanly and unzips the reports. Delete `/tmp/mimic_smoke_test` after: `rm -rf /tmp/mimic_smoke_test`.
