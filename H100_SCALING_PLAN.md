...
>
> **Current phase: Phase 10 — Image-conditioned report generator (on the 150M backbone). 10A + 10B-architecture DONE (2026-08-20); 10B-training onward still open, blocked on Phase 8's local MIMIC build finishing.**
...
- [x] **11C-infra** (2026-08-24) — **Retrieval-NN baseline built, NOT yet run.** `evaluate_report_generation.py --retrieval-baseline` (+ `scripts/retrieval_baseline_h100.sh`): retrieves the nearest training report by stock BiomedCLIP pooled-image cosine similarity (no Phase-9 best contrastive checkpoint exists yet — matches 10C's frozen-tower default) and emits it verbatim, scored by the same rouge_l/bleu/meteor as `--checkpoint` mode. Built after both the no-augmentation (job 2478647) and augmented (job 2482543) arm0 checkpoints were confirmed to generate byte-identical boilerplate rather than condition on the image — n-gram metrics reward templated text heavily and MIMIC reports are heavily templated (6C-3: 2% of the gallery is exact-duplicate). **This baseline is the real floor**; a generator that does not beat it has contributed nothing. **Run BEFORE trusting any generator ROUGE-L/BLEU number** — including the 0.28/0.33/0.12 already measured on arm0.
