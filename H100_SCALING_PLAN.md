...
>
> **Current phase: Phase 11 — Report-generation evaluation, full-data arm. Phase 8 (fetch+pack) and the full-data Phase 10E training run (job 2491338, 191,462 pairs) are DONE (2026-08-28). Full n=1433 eval (11C full, jobs 2491600/2491687) REVERSES the n=10 preliminary read: generator BEATS the retrieval-NN floor on all 3 metrics (rouge_l 0.2075 vs 0.1881; bleu_1 0.2706 vs 0.2605; bleu_4 0.0707 vs 0.0465). Caveat: 1055/1433 (73.6%) generated reports fall into one of 184 exact-duplicate template clusters — real signal but still heavily templated. 11B (CheXbert F1) harness BUILT 2026-08-28 (`--chexbert` flag, `f1chexbert`-backed, UNVERIFIED against real weights — this session has no cluster access) — the pure F1 math and CSV-loading are tested; the live labeler call is not. NEXT ACTION: `git pull` then `CHECKPOINT=./outputs/h100_report_gen_full/checkpoints/last.ckpt PARQUET=/sc/home/$USER/dataset/mimic_full/validate.parquet NUM_SAMPLES=1433 CHEXBERT=true sbatch scripts/inspect_report_generation_h100.sh` — this run IS the integration test. Also open, independent: 11D (official test split), 11E (qualitative appendix).**
> Phases 1/2/4/5/6/6B/6C/6D/6G/7/8 are **COMPLETE and CLOSED**. Phase 3 deferred (its lever was measured non-binding). Phase 9 (full-data retrieval attribution: Arm 0/1/2) is **DEFERRED by user decision (2026-08-20)** — the `9A-smoke (150M)` pipeline-health run was accepted as sufficient in place of the strict 70M `0.1459` reproduction; 9A/9B/9C remain open but are not being pursued right now. See `h100_scaling_state.json` notes for the full rationale.
...
- [x] **11B-infra** (2026-08-28) — **CheXbert F1 harness built, NOT yet run live.** `scripts/evaluate_report_generation.py` gained an opt-in `--chexbert` flag (usable with `--checkpoint`, `--retrieval-baseline`, or `--hyp-file`/`--ref-file`) plus `--chexpert-csv` for the ground-truth cross-check: `binary_f1_from_labels()` (micro/macro F1 over `CHEXPERT_14_LABELS` + the standard 5-label RRG subset `CHEXBERT_5_LABELS` — Atelectasis/Cardiomegaly/Consolidation/Edema/Pleural Effusion) is pure counting, CPU-unit-tested with synthetic label arrays (same split as 11C's `nearest_neighbor_indices` vs `embed_images`). `load_chexpert_ground_truth_labels()` parses `mimic-cxr-2.0.0-chexpert.csv.gz` (U-Zeros convention: 1.0→positive, {0.0,-1.0 uncertain,NaN}→not-positive) and is also unit-tested against a synthetic gzip CSV. The actual text→label step (`label_reports_with_chexbert`) is backed by the `f1chexbert` pip package (auto-downloads `chexbert.pth` on first use) — **UNVERIFIED AGAINST REAL WEIGHTS**, since this session has no cluster/GPU access to run it live; any import/download/API failure degrades to a printed "skipped" message and `None` (same contract as `meteor_score_corpus`'s nltk/wordnet guard), never crashes the surrounding ROUGE/BLEU. `inspect_report_generation_h100.sh` and `retrieval_baseline_h100.sh` both gained a `CHEXBERT=true`/`CHEXPERT_CSV=...` lever (default off, `set -u`-safe empty-array expansion). 12 new tests (F1 math, CSV parsing/binarization, graceful-skip-on-missing-package via `sys.modules[name]=None`, flag gating, SLURM wrapper conventions). `f1chexbert` added to `requirements.txt` as a guarded optional dep. **NEXT ACTION FOR THE USER (this IS the integration test):** `git pull`, then e.g. `CHECKPOINT=./outputs/h100_report_gen_full/checkpoints/last.ckpt PARQUET=/sc/home/$USER/dataset/mimic_full/validate.parquet NUM_SAMPLES=1433 CHEXBERT=true sbatch scripts/inspect_report_generation_h100.sh` (same checkpoint/data as the n=1433 ROUGE-L/BLEU run) — if `f1chexbert`'s API doesn't match what this code assumes, the log will show `CheXbert labeling skipped: <ExceptionType>: <message>` rather than a silent wrong number; report that message back if it skips.
