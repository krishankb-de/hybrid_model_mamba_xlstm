...
- [ ] **12A** — **STILL OPEN, genuine gap, not fabricated.** Authoritative MIMIC + Indiana retrieval numbers exist (quoted in 12B's writeup from the closed retrieval chapter), but **STS (BIOSSES/STS-B) and PubMed PPL on the best 150M ckpt were never actually measured anywhere in this project's history** — checked via grep across `H100_SCALING_PLAN.md`/`h100_scaling_state.json`, zero hits for BIOSSES/STS-B/PubMed-PPL results. `analysis/h100_scaling_results.md` (12B) does NOT claim these numbers exist; this checkbox stays open rather than being silently dropped. If the STS/PPL eval scripts (`scripts/evaluate_sts.py`, PubMed PPL eval) are run later, fold the results into `analysis/h100_scaling_results.md` §2.
