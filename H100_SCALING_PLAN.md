...
>
> **Current phase: Phase 11 — Report-generation evaluation, full-data arm. Phase 8 (fetch+pack) and the full-data Phase 10E training run (job 2491338, 191,462 pairs) are DONE (2026-08-28). Preliminary n=10 eval (11C full) shows the generator still below the retrieval-NN floor — leading hypothesis is undertraining (~1.7 effective epochs on the full set, vs arm0's ~16), not memorization. Next: rerun eval at NUM_SAMPLES=200 and check the loss trajectory for an undertraining signal before deciding whether to extend MAX_STEPS.**
...
- [ ] **11B** — **CheXbert F1**: run the CheXbert labeler over generated and ground-truth reports; report micro/macro F1 over the 14 CheXpert labels and the 5-label subset the RRG literature uses. `mimic-cxr-2.0.0-chexpert.csv.gz` (already downloaded in 8A) gives the ground-truth labels as a cross-check on the labeler wiring. Weights previously assumed to need pre-staging ("compute nodes have no internet") — **CONFIRMED WRONG 2026-08-24** via sc-helpdesk@hpi.de: compute nodes have a 1 Gbit/s internet uplink (shared, avoid saturating), matching what job 2461245/2482266's own PhysioNet fetch already proved live. Downloading CheXbert weights directly from an `aisc-batch` job should now just work — no separate staging step needed.
