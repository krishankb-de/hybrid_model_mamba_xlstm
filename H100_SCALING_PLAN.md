...
> Phases 1/2/4/5/6/6B/6C/6D/6G/7 are **COMPLETE and CLOSED**. Phase 8's `fetch` is 99.94% done (job 2482266, 2026-08-27) — one quick resubmit to mop up ~132 stragglers, then `pack` (never run for the full/non-arm0 build) is the last step before `cxr_mimic_full`'s parquet files exist. Phase 3 deferred (its lever was measured non-binding). Phase 9 (full-data retrieval attribution: Arm 0/1/2) is **DEFERRED by user decision (2026-08-20)** — the `9A-smoke (150M)` pipeline-health run was accepted as sufficient in place of the strict 70M `0.1459` reproduction; 9A/9B/9C remain open but are not being pursued right now. See `h100_scaling_state.json` notes for the full rationale.
