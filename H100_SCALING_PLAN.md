...
#### 15C — Auxiliary multi-label CheXpert loss ⏳ NOT STARTED — **gated on 15B-3/15B-5**
...
- [ ] **15C-1** — **Batch plumbing.** `chexpert_labels` (14-dim float tensor) added to `ImageTextDataset.__getitem__`, **opt-in and default off**. `ImageTextDataset` is shared with the CLOSED retrieval chapter, so the default path must stay behaviour-identical — the label lookup is constructed only when the report-gen config asks for it. `study_id` is available (`item = self.data[idx]` exposes every parquet column; 13F already reads `dataset.data["study_id"]`), and `cfg.dataset.chexpert_csv` already points at the on-cluster CSV.
...
- [ ] **15C-3** — **λ probe {0.1, 0.5} at seed 42, selected on `validate.parquet`, reported on `test.parquet`.** Single-lever: `cap` stays 10.0 across both arms, only `aux_lambda` moves. Validation-split selection is the discipline that closed §6.1 for `prefix_k`; repeating it here means 15C's dose is never chosen on the split it is reported on.
- [ ] **15C-4** — **3 seeds of the winning λ**, judged against 15B-3's seed band and 15B-5's per-label CIs. **Pre-registered decision rule, declared before the runs:** 15C is a **success** only if CheXbert-14-macro improves by more than the seed SD from 15B-3 **and** no CheXbert micro metric regresses below the retrieval floor (the precise failure that killed 13F at both doses). Anything else is reported as a negative or intermediate result.
  **⚠ RE-ANCHOR BEFORE 15C-3 LAUNCHES (flagged 2026-09-15, decision pending — rule not yet amended).** The band is now measured: hybrid 14-macro 0.2660 ± 0.0122 (range 0.2572–0.2800). Clause (b) as written can fail on seed noise alone — hybrid s44 clears the floor's 14-micro by only +0.0028 and Transformer s44 is below it. **Proposed amendment:** (a) aux 3-seed mean 14-macro > baseline 3-seed mean + 1 seed SD = **0.2782**, with the gain positive at ≥2 of 3 seeds paired by seed; (b) judged on the **3-seed mean** 14-micro vs the floor (0.4296), not per seed. The λ probe stays paired against the same-seed baseline on `validate.parquet`.
