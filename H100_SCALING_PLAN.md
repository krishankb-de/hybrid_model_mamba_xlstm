...
>
> ### ⚡ PLAN REOPENED AGAIN 2026-09-13 — PHASE 15 (second supervisor review, thesis strength) — ✅ **ALL FOUR ITEMS CLOSED 2026-09-16**. `current_phase: phase15_thesis_strength`.
> **Phase 14's three items are ACCEPTED and CLOSED.** A second review raised four more, scoped *"required for thesis strength — needed before submission, not urgent today"*: **(1)** a table of published field baselines (R2Gen, CvT2DistilGPT2, RGRG, RA-RRG, Janus-CXR) on the same metrics — never operationalised, zero published numbers exist in this repo; **(2)** close the CheXbert-14 macro gap (0.2800 vs ~0.40–0.42) via a multi-label CheXpert-class loss, since 13F's oversampling failed; **(3)** statistical rigor for the generation table — **no seeds, no CIs** where the retrieval table has SEs; **(4)** keep retrieval framed as a supporting chapter.
> **Two departures from the supervisor's ordering, both reasoned in Phase 15 below:** seeds (15B) run BEFORE the loss attempt (15C), because 13F's post-mortem says a single-run result is uninterpretable and the seed band is the measuring instrument; and 15A runs first because it calibrates 15C's target. **🔴 Computed before designing 15C: 0.40 macro is unreachable by fixing rare findings.** The retrieval floor scores nonzero F1 on all 14 labels (it copies real human reports) and still reaches only **0.3014** macro; lifting 13D's 3 zero-F1 labels to the floor's own values yields **0.3064**. Reaching 0.40 needs ~+0.12 on *every* label — a general-quality lever. **15C is authorised as ONE designed attempt reported honestly, not an iterate-to-target campaign.** Budget: **17 days** of H100 access (2026-09-13); Stage-0 needs no re-run, every Phase-15 arm is a report-gen decoder stage (hybrid 2h30m / Transformer ~40m).
>
> ### ⚡ PLAN REOPENED 2026-09-07 — PHASE 14 (supervisor review) — ✅ **ALL THREE ITEMS CLOSED**.
...
> ⚡ **14A HEAD-TO-HEAD COMPLETE (2026-09-09) — SPLIT DECISION.** ⚠️ **This line's original wording was overturned by the paired bootstrap and is corrected here (2026-09-13); the point estimates below are right but the verdicts they were given were not.** On the official test split (n=2663), with 95% paired-bootstrap CIs: the **hybrid significantly wins 3 of 4 CheXbert F1 metrics** — 14-micro 0.4736 vs 0.4590 [+0.0052,+0.0240], 5-micro 0.5522 vs 0.5249 [+0.0147,+0.0410], 5-macro 0.4487 vs 0.4319 [+0.0033,+0.0308] — while **14-macro is a TIE** (0.2800 vs 0.2774, CI spans zero). The **Transformer significantly wins ROUGE-L** (0.1936 vs 0.1899 [−0.0066,−0.0010]) **and exact-match-14**; **BLEU-1 and BLEU-4 are TIES**, not Transformer wins. So: hybrid 3, Transformer 2, three ties — *not* "all four vs all four" in either direction. CheXbert F1 asks whether the right *findings* were asserted; ROUGE/BLEU ask whether the *text* matches — and this plan designated CheXbert the more clinically meaningful metric back in Phase 11, when the hybrid was losing on it. The Transformer also wins Stage-0 PPL (11.222 vs 13.18) and **every** efficiency measure (14A-7), though the efficiency benchmark carries a large implementation confound. **Pre-registered quality bar: the CheXbert-14-micro half CLEARED, the ROUGE-L half FAILED, so as a conjunction it is NOT cleared** — see `analysis/PHASE14_SUPERVISOR_REVIEW.md` §2.5, which is authoritative over this summary line.
...
> 📄 **PRIMARY DELIVERABLE: `analysis/PHASE14_SUPERVISOR_REVIEW.md`** — the complete response to all three supervisor items, the head-to-head tables with 95% CIs, a detailed account of where the hybrid wins and why that axis is meaningful, the prefix-`k` sweep that shows k=32 is principled rather than favourable, the efficiency confound, all nine limitations, and reproduction commands. **Cite that file, not this plan, when answering the review.**
...
### Phase 15 — Thesis-strength block 🎯 NEW (2026-09-13) — second supervisor review

**Why:** Phase 14's three items were accepted. A second review (2026-09-13) raised four more, explicitly scoped as *"required for thesis strength — needed before submission, not urgent today"*. None of them disputes a Phase-14 number; all four are about what a thesis needs that a results log does not. **Budget: 17 days of H100 access remaining (stated by the user 2026-09-13).** Stage-0 does NOT need re-running for any item here — both 150M backbones already exist, so every run in this phase is a report-gen decoder stage (hybrid 2h30m / Transformer ~40m on 4×H100, measured jobs 2533151/2533152).

1. **No table of published field baselines** (R2Gen, CvT2DistilGPT2, RGRG, RA-RRG, Janus-CXR) on the same metrics. The `Target` tier has said *"competitive with R2Gen-class published baselines"* since Phase 1 and **was never operationalised — there is not one published number anywhere in this repository** (verified by grep 2026-09-13: 5 hits, all prose).
2. **Close the CheXbert-14 macro F1 gap.** 0.2800 vs ~0.40–0.42 in comparable recent systems, and below this project's own retrieval control (−7.1%). Driven by rare findings (Lung Lesion, Pleural Other) that are the most costly to miss clinically. Oversampling (13F) failed; **a multi-label CheXpert-class loss is the supervisor's nominated next attempt.**
3. **The generation table has no statistical rigor.** Table 2 (retrieval) reports standard errors and a 6.2-SE significance claim; Table 3 (generation) reports **no seeds and no CIs**. A single generator run against a single control is not defensible in a viva.
4. **Framing note on retrieval:** keep it a supporting chapter. It beats naive and off-the-shelf baselines and a dedicated MIMIC-CXR retrieval fine-tune, but trails the best specialised retrieval architecture — fine as supporting evidence, do not oversell.

**Two plan-level decisions taken before any of this runs, both departures from the supervisor's ordering:**

**(a) Seeds come BEFORE the loss attempt.** The supervisor listed macro-F1 second and rigor third; this plan inverts them. 13F's own post-mortem is the reason: *"without multi-seed averaging, this project cannot distinguish 'oversampling helps a little' from 'this run happened to land differently.'"* Running 15C before 15B's seed band exists would repeat 13F's exact failure — an uninterpretable single-run result. **The seed band is the measuring instrument for 15C, not an independent deliverable.**

**(b) 🔴 The macro target is arithmetically unreachable by the nominated mechanism, and this was computed before 15C was designed.** Using the retrieval floor's per-label breakdown (`results/retrieval_floor_test_split/chexbert_metrics.json`, read 2026-09-13):

- The floor scores **nonzero F1 on all 14 labels** — Lung Lesion 0.1419, Pneumothorax 0.1311, Pleural Other 0.0963, Fracture 0.0804 — because it copies a *real human report*, which sometimes mentions rare findings. That is the entire source of its macro edge.
- Floor summed per-label F1 = 4.219 → macro 0.3014 (reconciles with its own `macro avg`, 0.30136).
- 13D = 0.2800 × 14 = 3.920 summed, with 3 labels at exactly 0.0.
- Raise those 3 zeros to **the floor's own values** — a total success for 15C — and 13D reaches (3.920 + 0.369)/14 = **0.3064**.

**Reaching 0.40 needs +1.68 summed F1, i.e. ~+0.12 on every label — a general-quality lever, not a rare-finding fix.** And the calibration point that matters more: **a real radiologist's report, retrieved for a visually similar but different patient, scores 0.3014 macro under this exact evaluation.** Any published system reporting 0.40–0.42 is therefore very likely measuring something else. Establishing *what* is 15A's job, which is why 15A runs first — it calibrates 15C's target rather than the reverse. **15C is authorised as ONE designed attempt reported honestly (user decision 2026-09-13), not an iterate-to-target campaign.**

**Execution order:** `15A` + `15B-1` + `15B-2` + `15D` immediately (all local or one cheap job) → **`15B-3` seed arms queued as the long pole** → `15C` code written while they run → `15C` probe → `15C` final.

---

#### 15A — Published field-baseline table ✅ **DONE 2026-09-16** (`analysis/PUBLISHED_BASELINES.md`; §4.2 is the verdict). Two sourcing gaps remain **stated as open, not filled**: Janus-CXR (paywalled — DO NOT CITE) and RA-RRG's BLEU/ROUGE-L (column misalignment).

**Deliverable:** `analysis/PUBLISHED_BASELINES.md`, referenced from `analysis/PHASE14_SUPERVISOR_REVIEW.md` and `analysis/h100_scaling_results.md`.

**🔴 Standing honesty rule for this block, non-negotiable:** every number carries a citation to a specific paper/table. Anything that cannot be sourced is written as `unverified — do not cite`, **never** filled in from recollection. A wrong digit in a baseline table is fatal in a viva, and it is the one failure mode here that cannot be caught by any test in this repo.

- [x] **15A-1** — **DRAFTED 2026-09-13.** §3.1 (example-F1 lineage, from PromptMRG's MIMIC-CXR table) and §3.2 (macro/micro LLM-scale tier, from RA-RRG Table 1). **Verdict: the supervisor is right on macro** — 0.2800 is below every modern system except DCL (0.284) — **but every system in the 0.386–0.417 band is 7B–84B params or retrieval-augmented, 38–460× this project's 183M.** Nearest parameter-comparable: PromptMRG ~0.2B at macro 0.381. **Sourcing rule caught two errors:** a search returned R2Gen/CvT2DistilGPT2/RGRG macro 13.6/16.8/18.0 as MIMIC-CXR — they are **IU X-Ray**; and RA-RRG's NLG columns could not be aligned (one reading gave BLEU-4 > BLEU-1), so they are blank. **OPEN:** ~~this project's own example-F1~~ **CLOSED 2026-09-16 — read out for all 6 arms and now in §3.1/§3.2: hybrid 0.3790 ± 0.0214 (3 seeds), Transformer 0.3817 ± 0.0144, floor 0.3691. That places the project between M2TR (0.308) and M2KT/MKSG (0.352/0.371) on the R2Gen-lineage table, and — the line worth making — within one seed SD of PromptMRG's parameter class on macro's sister metric while the raw macro gap stays wide**; Janus-CXR (paywalled, marked DO NOT CITE); RA-RRG BLEU/ROUGE-L. Original spec: Rows: R2Gen, CvT2DistilGPT2, RGRG, RA-RRG, Janus-CXR + this project's hybrid (13D), the matched Transformer, and the retrieval-NN floor. Metrics: ROUGE-L, BLEU-1, BLEU-4, CheXbert-14 micro/macro, CheXbert-5 micro/macro where published.
- [x] **15A-2** — **DONE 2026-09-13** (§2 of the doc). The comparability columns are the point of the table, not decoration.** Record for every row: eval split (official MIMIC-CXR test vs custom), labeler (CheXbert vs CheXpert-labeler vs CheXpert-labeler-on-14), label set (14 vs 5), **target text**, and views (frontal-only vs all).
  **The `target text` column is the load-bearing one.** This project's references are `"Findings: {findings} Impression: {impression}"` concatenated (`scripts/train_contrastive.py:230`, `concatenate_sections: true`), max_length 256, frontal only. Most R2Gen-class work generates **findings only**. Different target text makes ROUGE-L/BLEU non-comparable and plausibly macro-F1 too. **Note also that this plan's own "Unresolved questions" has carried `Report-gen text target: findings-only (RRG convention) vs findings+impression` as an OPEN item since Phase 8F and never resolved it** — that unresolved decision is precisely what 15A now has to report the consequence of.
- [x] **15A-3** — **DONE 2026-09-16 — `analysis/PUBLISHED_BASELINES.md` §4.2.** The answer is **split, and the split is the finding**: on **macro-F1 the gap survives and widens** (0.2800 → 0.2660 ± 0.0122 after seeds; every other adjustment is neutral or unquantified; the shortfall to the only parameter-comparable system, PromptMRG ~0.2B at 0.381, is **0.115**), while **on example-F1 — the metric four of the five supervisor-named systems actually report — it does not**: 0.3790 ± 0.0214 sits level with CvT2DistilGPT2 (0.384) and above R2Gen, M2TR, METransformer, KiUT, M2KT and MKSG. The two metrics disagree because macro averages 14 labels unweighted and is dominated by the rarest, which is the same arithmetic §4.1 uses. Both pre-registered possibilities were therefore realised, one per metric. Original spec: Write the verdict: state where this project genuinely stands, where the comparison is apples-to-oranges and why, and how much of the "0.28 vs 0.40" macro gap survives a like-for-like reading. Pre-register the honest possibility that **the gap does not fully survive** and the possibility that **it does** — do not write either conclusion before the sourcing is done.

#### 15B — Statistical rigor for the generation table ✅ **DONE 2026-09-16** — all five items. 🔴 **The seed band overturns Phase 14's architecture verdict in BOTH directions and softens 15B-1's floor claim.** Supervisor item 3 is answered: the generation table now carries 3 seeds × both arms, paired-by-seed differences with SDs, and per-seed bootstrap CIs.

- [x] **15B-1** — **DONE 2026-09-13 (re-score job 2542358, bootstrap job 2542384; `analysis/bootstrap_hybrid_vs_retrieval_floor.md`).** n=2663, 1000 paired resamples. **13D wins 7 with CI excluding zero** — ROUGE-L +0.0263 [+0.0228,+0.0293], BLEU-1 +0.0097, BLEU-4 +0.0212 [+0.0181,+0.0242], CheXbert-14-micro +0.0440 [+0.0321,+0.0561], CheXbert-5-micro +0.0665 [+0.0511,+0.0827], CheXbert-5-macro +0.0203 [+0.0039,+0.0380], exact-match-5 +0.0428. **The floor wins 1: CheXbert-14-macro −0.0214 [−0.0340,−0.0081].** exact-match-14 ties (lower bound exactly +0.0000). **The macro deficit is now interval-backed, not a point estimate** — it confirms supervisor item 2 with a measurement. Original spec: **CIs on generator-vs-retrieval-floor.** This is the literal "Table 3 reports none" complaint. **Zero new code** — `scripts/bootstrap_compare.py` already takes any two hyp dumps. Blocker confirmed 2026-09-13: `results/retrieval_floor_test_split/` has `hyps.txt`/`refs.txt`/`chexbert_metrics.json` but **no `chexbert_labels.json`** (it was scored 2026-08-30, before `--dump-labels` existed), so re-score it once (~15 min) to get the label matrices, then bootstrap. **⚠ SUPERSEDED IN PART 2026-09-16 by the seed replication (jobs 2546525/2546526, `analysis/bootstrap_hybrid_seed4{3,4}_vs_retrieval_floor.md`).** Across seeds 42/43/44 the generator's win over the floor is **robust on text and exact-match** (ROUGE-L, BLEU-1, BLEU-4, exact-match-5: CI excludes zero at all 3 seeds; exact-match-14 wins at s43, ties at s42/s44, never loses) but **NOT on CheXbert**: 14-micro +0.0440 / +0.0084 / +0.0029 — significant at seed 42 only, a tie at the other two; 5-micro likewise (+0.0665 / −0.0047 / +0.0071); 5-macro flips (+0.0203 / −0.0339 / −0.0138). **14-macro loses at all three seeds** (−0.0214 / −0.0441 / −0.0404). So "13D wins 7 of 9 vs the floor" is a seed-42 statement; the seed-robust statement is **"better text, equal clinical micro-F1, worse macro-F1 than copying a neighbour's report."**
- [x] **15B-2** — **DONE 2026-09-13 (commit be3e8b8).** `SEED` on `train_report_generation_h100.sh`, default 42, passed as `seed=${SEED}`, echoed as `Training seed: N`; parity test guards all three. **Verified live** in jobs 2542397/2542912/2542913/2542914. Original spec: **Add a `SEED` lever.** `configs/config.yaml:13` hardcodes `seed: 42` and **no wrapper exposes it** — every report-gen run in this project's history is one seed. Add `SEED` to `scripts/train_report_generation_h100.sh` (passing `seed=${SEED}`) + a parity test asserting the lever exists, same shape as the existing `PREFIX_K`/`OVERSAMPLE_*` lever tests.
- [x] **15B-3** — **DONE 2026-09-15.** Generation jobs 2544442–2544445 (all four logs: `prefix_k = 32`, `Missing keys: 0, Unexpected: 0`); CheXbert scoring jobs 2545312–2545315 (n=2663). **Recipe parity with seed 42 verified, not assumed:** 13D's training log (job 2502981, 2026-09-02) and seed 43's (2542397) differ in their first 60 lines only by date, node, GPU temperatures and the seed lines, and both end `` `max_steps=12000` reached ``. Every training-path commit since 13D is behaviour-neutral for these arms: 33ca7f8 (additive `attention` layer type; `HYBRID_EXACT_SCAN` opt-in, off), fc2471e (decoder-init guard, check only), be3e8b8 (`SEED`, default 42), da27523 (`SAVE_TOP_K`). So seed 42 is a legitimate draw of the same recipe and **stays in the pool** — dropping the high draw would be cherry-picking in the other direction. Eval-path commits since 13D's decode (14e683a label dump, 90099ad prefix_k resolution) are tested empirically instead: 13D re-decoded with today's code into `results/report_gen_tower13d_redecode_test_split`, `cmp` against the original `hyps.txt`. Training history: hybrid s43 job 2542397 (2h22m, `val_lm_loss` 1.0462 @12000); hybrid s44 job 2542912 (2h15m); Transformer s43 job 2542913 (26m31s); Transformer s44 job 2542914 (26m30s). All exit 0:0, one `last.ckpt` each. **🔴 First attempt lost 3 of 4 arms to the home quota (195.7/200 GiB):** jobs 2542398/2542400 FAILED (exit 9/1 at ~42 min) and 2542399 died with `OSError: [Errno 122] Disk quota exceeded` on the `last.ckpt` write. The `Invalid cross-device link` line above it is a red herring — `shutil.move` falls back to copy across devices and it was the copy that hit the quota. Root cause: `save_top_k=3` + `last.ckpt` = 4 × 2.4 GB = 9.6 GB per arm, while **no eval in this project has ever loaded a top-k checkpoint**. Fixed with a `SAVE_TOP_K` lever (commit da27523, default 3); re-runs used `SAVE_TOP_K=0`, which also cut wall time (Transformer ~37m→26m, hybrid 2h30m→2h15m). 28.6 GB of top-k files plus superseded arms reclaimed; the retrieval chapter's checkpoints were **deliberately kept** (D0/D1c/cleansplit/6g7_vit2_cleansplit back the headline 11.07/17.14/14.59/10.81% numbers). **Checkpoint-consistency note, recorded as a stated property:** with `SAVE_TOP_K=0` each arm keeps only `last.ckpt` (step 12000), not the best-by-val-loss step. That is **consistent across all six arms** — seed 42's 13D and Transformer were also always evaluated from `last.ckpt` — and the gap is immaterial where measured (seed 43: last 1.0462 vs best 1.0460 @11750). Original spec: **Seeds 43 and 44 × both arms.** Seed 42 already exists for both (13D and the 14A Transformer), so this is **2 new hybrid runs (5h) + 2 new Transformer runs (1h20m)**, then generate + score each on the official test split. Matched seeds across arms **by design** — it makes 15B-4's comparison paired rather than unpaired.
- [x] **15B-4** — **DONE 2026-09-16. Report mean ± SD per metric per arm, plus the hybrid−Transformer difference paired by seed.** The question a viva will actually ask: *is the +0.0146 CheXbert-14-micro win larger than seed noise?* **Pre-registered honesty constraint: 3 seeds licenses "positive in k/3 seeds, range [x, y]" and a mean ± SD. It does NOT license a p-value** (2 df), and no t-test will be reported. Two distinct uncertainty sources must be kept visibly separate in the writeup: bootstrap CI = *sampling* uncertainty over studies; seed SD = *training* uncertainty over runs. They are not interchangeable and neither subsumes the other.
  **DONE 2026-09-16.** Sources: seed 42 from `h100_scaling_state.json`'s 14A block (re-verified — see the re-decode check below); seeds 43/44 from jobs 2546523/2546524 (`analysis/bootstrap_hybrid_vs_transformer_seed4{3,4}.md`). n=2663, beam 3, k=32, `last.ckpt`, matched seeds so every difference is paired.

  **Seed-42 reproducibility, checked not assumed (job 2546521):** 13D re-decoded with today's code produced a `hyps.txt` **byte-identical** (`cmp`) to the 2026-09-03 dump. The eval path has not drifted, so the seed-42 row is a real draw of the same recipe and stays in the pool.

  | metric | hybrid 42 / 43 / 44 | mean ± SD | Transformer 42 / 43 / 44 | mean ± SD | hybrid − Transformer, paired | mean ± SD | per-seed CI verdicts |
  |---|---|---|---|---|---|---|---|
  | ROUGE-L | .1899 / .1991 / .1957 | .1949 ± .0047 | .1936 / .1976 / .1945 | .1952 ± .0021 | −.0038 / +.0015 / +.0012 | −.0004 ± .0030 | T wins s42; tie, tie |
  | BLEU-1 | .2469 / .2524 / .2531 | .2508 ± .0034 | .2496 / .2484 / .2453 | .2478 ± .0022 | −.0027 / +.0039 / +.0079 | +.0030 ± .0054 | tie; **H wins s43, s44** |
  | BLEU-4 | .0542 / .0604 / .0588 | .0578 ± .0032 | .0571 / .0593 / .0562 | .0575 ± .0016 | −.0029 / +.0011 / +.0026 | +.0003 ± .0028 | tie, tie; **H wins s44** |
  | CheXbert-14-micro | .4736 / .4380 / .4324 | .4480 ± .0223 | .4590 / .4455 / .4285 | .4443 ± .0153 | +.0146 / −.0075 / +.0040 | +.0037 ± .0111 | **H wins s42**; tie, tie |
  | CheXbert-14-macro | .2800 / .2572 / .2609 | .2660 ± .0122 | .2774 / .2730 / .2572 | .2692 ± .0106 | +.0026 / −.0157 / +.0037 | −.0031 ± .0109 | tie; **T wins s43**; tie |
  | CheXbert-5-micro | .5522 / .4809 / .4928 | .5086 ± .0382 | .5249 / .5049 / .4797 | .5032 ± .0227 | +.0273 / −.0240 / +.0131 | +.0055 ± .0265 | **H wins s42**; **T wins s43**; tie |
  | CheXbert-5-macro | .4487 / .3945 / .4146 | .4193 ± .0274 | .4319 / .4198 / .3992 | .4170 ± .0165 | +.0168 / −.0253 / +.0154 | +.0023 ± .0239 | **H wins s42**; **T wins s43**; tie |
  | exact-match-14 | .0349 / .0447 / .0345 | .0380 ± .0058 | .0469 / .0473 / .0424 | .0455 ± .0027 | −.0120 / −.0026 / −.0079 | −.0075 ± .0047 | **T wins s42**; tie, tie |
  | exact-match-5 | .2163 / .2144 / .2182 | .2163 ± .0019 | .2306 / .2234 / .2193 | .2244 ± .0057 | −.0143 / −.0090 / −.0011 | −.0081 ± .0066 | **T wins s42**; tie, tie |
  | example-F1 (`samples avg`) | .4029 / .3726 / .3615 | .3790 ± .0214 | .3951 / .3835 / .3664 | .3817 ± .0144 | +.0078 / −.0109 / −.0049 | −.0027 ± .0095 | not bootstrapped |

  **What this licenses — and what it retracts:**
  1. **🔴 Phase 14's split decision does not replicate, in BOTH directions.** The hybrid's three CheXbert wins were seed 42 only (at seed 43 the Transformer wins 14-macro/5-micro/5-macro; at seed 44 nothing is called). The Transformer's ROUGE-L win was also seed 42 only (ties at 43/44, both nominally the hybrid's way). Every paired mean sits inside one paired SD. **Licensed claim: at matched parameters and k=32 the two architectures are statistically indistinguishable on 8 of 10 metrics across 3 seeds.**
  2. **The two survivors are small and opposite.** BLEU-1: hybrid ahead at 2/3 seeds with CI excluding zero at both new seeds (paired mean +.0030 ± .0054). Exact-match (5- and 14-label): **Transformer ahead at 3/3 seeds** (paired means −.0081 ± .0066 and −.0075 ± .0047), though only seed 42's CI excludes zero. Neither is strong enough to headline; both are worth one honest sentence.
  3. **Seed 42 is the high draw for CheXbert and the LOW draw for ROUGE-L** — for *both* architectures. That is the signature of a training draw, not of a recipe difference, and the byte-identical re-decode rules out eval drift. Consistent with selection: 13D was picked as the best 13-series arm, and the same-seed k=8/k=64 hybrids (0.4481/0.4488) sit with seeds 43/44 — so **Phase 14 §6's "k=32 is a significant, bracketed optimum" is very likely the same draw**. Flag it in the writeup; do not re-run the sweep (the finding it supports, "the conditioning interface matters", does not depend on the exact optimum).
  4. **Headline numbers become mean ± SD**, never 13D alone: hybrid ROUGE-L 0.1949 ± 0.0047, CheXbert-14-micro 0.4480 ± 0.0223, 14-macro 0.2660 ± 0.0122, example-F1 0.3790 ± 0.0214.
  5. **Seed SD is 3–15× the width the bootstrap CIs implied.** CheXbert-14-micro: seed SD 0.0223 vs a bootstrap half-width of ~0.0095. This is the concrete answer to the supervisor's *"a single generator run vs a single control isn't enough evidence"* — quantified, in this project's own numbers.

- [x] **15B-5** — **DONE 2026-09-14 (commit 6556410).** `bootstrap_compare.py --per-label` (`PER_LABEL=true` on the wrapper): per-label CheXbert-14 F1 with 95% CIs in a **separate** section, sorted by effect size, ending in an explicit "labels that moved" line. Zero extra cost — macro is computed from the same per-label list. Tests pin per-label F1 to sklearn `average=None`, pin its mean to macro at 1e-12, and assert enabling the flag adds rows but never changes an existing number. Smoke-tested with a planted rare-label signal: only the planted label flagged. Original spec: **Per-label F1 confidence intervals** added to `bootstrap_compare.py` (the `chexbert_labels.json` matrices already carry everything needed; this is a loop over 14 columns, not new measurement). This is the instrument that decides 15C: without per-label CIs, a rare-label movement cannot be distinguished from noise — which is exactly how 13F ended up uninterpretable.

#### 15C — Auxiliary multi-label CheXpert loss 🔴 **NEGATIVE RESULT 2026-09-16 — the pre-registered stop rule fired.** Writeup: **`analysis/PHASE15C_AUX_LOSS.md`** (15C-5) — cite that, not summary lines here. Code shipped (15C-1/15C-2, harness 9/9); λ probe run (15C-3); **neither λ improved validate CheXbert-14-macro, so 15C-4's 3 seeds do NOT run** — one designed attempt, reported honestly (user decision 2026-09-13).

**Mechanism decision (recorded with the two rejected alternatives, because the mechanism matters more than the loss function here).** An auxiliary head is **discarded at decode time**, so it can only change behaviour by reshaping representations — which is why *where* it attaches decides whether it can work at all.

| option | attach point | verdict |
|---|---|---|
| **A** | mean-pooled `prefix_embeds` (B,32,768) → `Linear(768,14)` | ✅ **CHOSEN** |
| B | decoder's pooled final hidden state | ❌ rejected on principle — under teacher forcing the pooled report representation *already contains the report*, so the aux task is solvable from the text alone and exerts **no grounding pressure** |
| C | token-level CE upweighting on rare-finding mention spans | ❌ rejected — needs a brittle string-match span detector, reproduces 13F's "help the rare, hurt the common" shape, and is not the nominated mechanism |

**Why A is targeted rather than arbitrary:** the prefix is currently trained by the LM loss *alone*, so a finding that rarely appears in text sends almost no gradient into the connector — a concrete, nameable bottleneck. It is also the component this project's own most robust finding identifies as dominant: `analysis/PHASE14_SUPERVISOR_REVIEW.md` §3.5, *"the conditioning interface matters more than the sequence-mixing architecture"* (the k=8→32 swing, +0.0255, exceeds every architecture difference measured at matched k). **§3.5 was written 2026-09-12, before this item existed** — so this is targeting a pre-existing finding, not a post-hoc rationalisation.

**Fairness property worth stating in the writeup:** the head is ~10.7k params and **lives outside the evaluated model**. `scripts/evaluate_report_generation.py:306` loads with `strict=False`, so `aux_head.*` lands in `unexpected` and is ignored, and the guard at line 321 inspects only `missing`. **The evaluated network stays parameter-identical to 13D**, so 14A's matched-parameter claim survives 15C intact.

- [ ] **15C-0** — **TARGETING, from the per-seed per-label bootstraps vs the floor (2026-09-16, jobs 2546525/2546526).** The two new seeds agree on *where* the macro deficit lives, so this is structure, not noise. Hybrid − floor per-label F1 (s43 / s44):
  | label | support | hybrid s43 / s44 | floor | diff s43 / s44 |
  |---|---|---|---|---|
  | **Lung Opacity** | **1027** | .2189 / .1860 | .4011 | **−.1822 / −.2151** (both CIs exclude 0) |
  | Lung Lesion | 177 | .0112 / .0000 | .1419 | −.1307 / −.1419 |
  | Pneumonia | 549 | .1126 / .1331 | .2102 | −.0976 / −.0771 |
  | Atelectasis | 958 | .3449 / .3759 | .4423 | −.0974 / −.0664 |
  | Pleural Other | 111 | .0000 / .0000 | .0963 | −.0963 / −.0963 |
  | Pneumothorax | 84 | .0392 / .0331 | .1311 | −.0919 / −.0981 |
  | Support Devices | 1192 | .7300 / .7372 | .6195 | **+.1105 / +.1177** (hybrid wins) |
  | Enl. Cardiomediastinum | 708 | .3087 / .2839 | .2147 | **+.0940 / +.0692** (hybrid wins) |
  | Cardiomegaly | 1159 | .6009 / .5877 | .5431 | **+.0578 / +.0446** (hybrid wins) |

  **🔴 This reframes 15C.** The single largest macro deficit is **Lung Opacity — a label with 1027 supports, not a rare finding at all** (21% of the total −0.617 summed-F1 deficit at s43; Lung Opacity + Lung Lesion = 36%). The supervisor's framing (*"driven by rare findings — Lung Lesion, Pleural Other"*) is only half right: rare labels contribute, but the biggest single loss is a common label the model under-mentions. A `pos_weight`-ed multi-label loss is still the right instrument — it just should not be justified as a rare-label fix alone, and **Lung Opacity / Pneumonia / Atelectasis are the labels to watch in 15B-5's per-label CIs**.
- [x] **15C-1** — **DONE 2026-09-16.** `load_chexpert_label_matrix()` + `CHEXPERT_14_LABELS` in `scripts/train_report_generation.py`; `ImageTextDataset(chexpert_labels=…)` in `scripts/train_contrastive.py`, **default `None`** so the closed retrieval chapter's batch payload is unchanged (a test asserts `set(batch) == {input_ids, attention_mask, pixel_values}` at the default). Targets are attached only when `model.aux_lambda > 0`. **A study with no CheXpert row is absent from the matrix and arrives with `chexpert_label_mask = 0.0`,** so the aux loss skips it rather than learning "no findings" from missing data. Label order is pinned by a spec-lock test against f1chexbert's `target_names` — a permuted target would train the head on the wrong findings while every loss curve still looked healthy. Original spec: **Batch plumbing.** `chexpert_labels` (14-dim float tensor) added to `ImageTextDataset.__getitem__`, **opt-in and default off**. `ImageTextDataset` is shared with the CLOSED retrieval chapter, so the default path must stay behaviour-identical — the label lookup is constructed only when the report-gen config asks for it. `study_id` is available (`item = self.data[idx]` exposes every parquet column; 13F already reads `dataset.data["study_id"]`), and `cfg.dataset.chexpert_csv` already points at the on-cluster CSV.
  **U-Zeros convention** (`1.0` → positive; `{0.0, −1.0, NaN}` → negative), matching `compute_rare_finding_sample_weights()` verbatim **so 15C and 13F remain directly comparable** (user decision 2026-09-13, chosen over masking uncertains out). CPU-unit-testable against the same tiny on-disk CSV fixture 13F's tests already use.
- [x] **15C-2** — **DONE 2026-09-16.** `aux_head` + masked `BCEWithLogitsLoss` in `ReportGenerationLightningModule`, `set_aux_pos_weight()` called from `train_report_generation.py` with prevalence from the **train split only**. `AUX_LAMBDA` / `AUX_POS_WEIGHT_CAP` levers on the wrapper (defaults 0.0 / 10.0), echoed positively as `Aux CheXpert loss: lambda=… cap=…` plus a per-label `pos_weight` dump. **Zero-behaviour-change proved by test, not asserted:** at `aux_lambda=0.0` the head is never constructed, so no `aux_head.*` key enters the checkpoint **and the decoder/prefix_mapper init is bit-identical** to a module built before this code existed (the head is built LAST because constructing an `nn.Linear` consumes global-RNG draws — that is what would have silently invalidated the 15B-4 baseline). A further test loads an aux state dict into a no-aux module and asserts `missing == []` with every `aux_*` key in `unexpected`, which is the fairness property the writeup claims. `val/lm_loss` is still logged before the aux term, so **`ModelCheckpoint` selection remains the pure LM criterion** every prior arm used. The head is in the optimiser at `head_lr` — omitted, it would stay at random init and the aux gradient reaching the connector would be noise. Original spec: **`aux_head` + loss in `ReportGenerationLightningModule`.** `total = lm_loss + aux_lambda * BCEWithLogitsLoss(pos_weight=w)(head(prefix.mean(1)), labels)`. **`aux_lambda` defaults to 0.0, so shipping this code changes nothing** — same zero-behaviour-change discipline as 13F's `oversample_rare_findings: false`. Log `train/aux_loss` and `val/aux_loss` separately from `lm_loss`; **`val/lm_loss` stays the `ModelCheckpoint` monitor** so checkpoint selection remains comparable with every prior arm.
  `pos_weight = min((N − n_pos)/n_pos, cap)`, **`cap` default 10.0**: uncapped, Pleural Other's ~1% prevalence gives ~100×, and 13F demonstrated that 5× already damaged 4+ common high-support labels. Computed once from the train split and logged.
- [x] **15C-3** — **DONE 2026-09-16 — BOTH DOSES FAIL THE SELECTION RULE.** λ probe {0.1, 0.5} at seed 42, selected on `validate.parquet`, reported on `test.parquet`.

  Smoke job 2548455 (aux path live: head built, 100% CheXpert coverage, cap binds on the 6 rarest labels, `aux_loss`/`total_loss` scalars confirmed present in tfevents).
  Training jobs **2548850** (λ=0.1) and **2548851** (λ=0.5), 4 GPUs, 12000 steps, ~2h15 each, `SAVE_TOP_K=0`. Decode 2550145/2550153, scoring 2550637/2550638.
  Baseline is **13D itself** on the same split and decode (`results/report_gen_tower13d_n1433`, job 2503223, 2026-09-02) — same seed 42, same recipe, so this is paired.
  ...
  | example-F1 | *(pending)* | 0.3826 | — | 0.3771 | — |

  **Verdict against the rule declared before the runs** (*"the λ with the higher CheXbert-14-macro on validate wins… if neither beats 13D's validate macro, that is the reported answer and 15C-4 does not run"*): **neither λ wins.** λ=0.1 is a wash — −0.0016 is an eighth of the 15B-4 seed SD (0.0122), i.e. indistinguishable from doing nothing. λ=0.5 is **harmful** on every metric, and the dose-response is monotone downward, which is itself evidence the mechanism is not merely under-dosed.

  **Per-label, the failure has 13F's exact shape** (validate, CheXbert-14 F1): at λ=0.1 the two labels the aux loss weights hardest are **still exactly 0.000** (Lung Lesion, Pleural Other) — a `pos_weight` of 10 on the prefix head did not buy a single mention. At λ=0.5 they move off zero (0.018 / 0.041) but **Consolidation and Fracture collapse to 0.000**, and Lung Opacity — the biggest macro deficit vs the floor (15C-0) — drops 0.257 → 0.238. Help the rare, hurt the rest: the same trade 13F bought with oversampling, now bought with a loss term instead.
  ...
  **TEST-SPLIT CIs, per-label (job 2551233, `analysis/bootstrap_aux_lam0.1_vs_13d_test.md`).** CheXbert: **every metric ties** — 14-macro +0.0020 [−0.0072,+0.0110], 14-micro −0.0021, 5-micro −0.0027, 5-macro −0.0014, exact-match-5 +0.0124 [−0.0019,+0.0252]. The only calls go to the **baseline**: ROUGE-L −0.0024 [−0.0048,−0.0003] and BLEU-1 −0.0026 [−0.0050,−0.0003]. So the aux loss bought nothing and cost a little text.

  **Per-label, the mechanism is visible but ~10× too weak.** Four of the five largest positive moves are exactly the rare labels the `pos_weight` cap targets — Pneumonia +0.0270, Pneumothorax +0.0234, Lung Lesion +0.0109 (off zero), Fracture +0.0107 — and **the one label that moves significantly is a common one going down: Support Devices −0.0153 [−0.0304,−0.0004]**, the highest-support label in the set. Direction right, magnitude an order of magnitude short of the −0.044 macro deficit vs the floor, and the common-label loss cancels most of the rare-label gain (net macro +0.0020).

  ⚠ **The validate bootstraps carry text metrics only.** `results/report_gen_tower13d_n1433` predates the label-dump fix (14e683a, 2026-09-09), so it has no `chexbert_labels.json` and the CheXbert rows were skipped with a warning. **Re-scored 2026-09-16 (job 2551261) — `chexbert_labels.json` now exists**, so those two bootstraps can be regenerated with CheXbert CIs; the point estimates above are unaffected (13D validate macro 0.28688 confirms the 0.2869 used for selection).

  Original spec:

  **Recipe (2026-09-16), baseline = 13D itself (seed 42, same recipe, `results/report_gen_tower13d_n1433`):**
  ```bash
  # 0. smoke first -- the aux path has never run on GPU (50 steps, 1 GPU, ~15 min).
  #    Confirm in the log: "Aux CheXpert loss: lambda=0.1", a coverage line
  #    "[aux] train: CheXpert targets cover N/M rows", and 14 pos_weight lines.
  SEED=42 SAVE_TOP_K=0 AUX_LAMBDA=0.1 MAX_STEPS=50 \
  IMAGE_ENCODER_CKPT=./outputs/h100_kd_150m_v2_full_data_lr3e6/checkpoints/last.ckpt \
  EXPERIMENT=h100_report_gen_aux_smoke sbatch scripts/train_report_generation_h100.sh

  # 1. the two probe arms (4 GPUs each, ~2h15 each)
  ...
  # 2. decode + score on VALIDATE (n=1433), never on test
  for LAM in 0.1 0.5; do
    MODEL_CONFIG=hybrid_150m_v2_rrg DECODE=beam BEAM_SIZE=3 \
    PARQUET=/sc/home/$USER/dataset/mimic_full/validate.parquet NUM_SAMPLES=999999 \
    DUMP_DIR=results/report_gen_aux_lam${LAM}_validate \
    CHECKPOINT=./outputs/h100_report_gen_aux_lam${LAM}_seed42/checkpoints/last.ckpt \
      sbatch scripts/inspect_report_generation_h100.sh
  done
  ```
  Selection rule, declared before the runs: **the λ with the higher CheXbert-14-macro on validate wins**; ties broken by 14-micro. If *neither* λ beats 13D's validate macro, that is the reported answer and 15C-4 does not run — one designed attempt, not iterate-to-target (user decision 2026-09-13).
  Single-lever: `cap` stays 10.0 across both arms, only `aux_lambda` moves. Validation-split selection is the discipline that closed §6.1 for `prefix_k`; repeating it here means 15C's dose is never chosen on the split it is reported on.
- [x] **15C-4** — **NOT RUN, BY THE PRE-REGISTERED RULE (2026-09-16).** No λ won the validate selection, so there is nothing to replicate across seeds; running seeds anyway, or probing further doses, would be exactly the iterate-to-target behaviour this item was written to forbid. The ~7 GPU-hours are returned to the budget (the account expires 2026-09-29). Original spec: **3 seeds of the winning λ**, judged against 15B-3's seed band and 15B-5's per-label CIs. **Pre-registered decision rule, declared before the runs:** 15C is a **success** only if CheXbert-14-macro improves by more than the seed SD from 15B-3 **and** no CheXbert micro metric regresses below the retrieval floor (the precise failure that killed 13F at both doses). Anything else is reported as a negative or intermediate result.
  **✅ RE-ANCHORED AND ADOPTED 2026-09-16 (user delegated the call; the amendment below REPLACES the clause above, and is fixed before any aux arm runs).** The band is now measured: hybrid 14-macro 0.2660 ± 0.0122 (range 0.2572–0.2800). Clause (b) as written can fail on seed noise alone — hybrid s44 clears the floor's 14-micro by only +0.0028 and Transformer s44 is below it. **The rule:** (a) the **paired** (aux − baseline, same seed) CheXbert-14-macro mean exceeds **+0.0122** — one baseline seed SD — **and** is positive at ≥2 of 3 seeds; (b) the aux arm's **3-seed mean** CheXbert-14-micro does not fall below the retrieval floor's 0.4296. Clause (b) is judged on the mean, not per seed, because at seed 44 even the baseline clears the floor by only +0.0028 — a per-seed clause would fail on noise the aux loss did not cause. Anything short of both clauses is written up as a negative or intermediate result.
- [x] **15C-5** — **DONE 2026-09-16 — `analysis/PHASE15C_AUX_LOSS.md`.** Reports the mechanism and why the prefix was chosen over the decoder's pooled state, the pre-registered rule and the fact that it fired, both splits, the per-label evidence that the intervention moved the intended labels in the intended direction ~10× too weakly, and the three independent signals that **grounding a representation is not the same as changing what gets written** (train loss untouched; text metrics tie at both doses; the head learns while Lung Lesion/Pleural Other stay at ~zero F1 in the generated text). Names what is NOT ruled out — label-conditioned *decoding*, PromptMRG's actual mechanism — as the honest next step rather than a claim. Original spec: Write it up either way. **A negative here is worth more than 13F's was**, and for a stateable reason: 15B's seed band makes "smaller than seed variance" a real, quantified finding, where 13F could only report "non-monotonic across two doses" and could not separate effect from noise.

#### 15D — Retrieval framing ✅ **DONE 2026-09-16** (edits only, no compute)

- [x] **15D-1** — **DONE 2026-09-16.** `analysis/h100_scaling_results.md` §2 now carries the framing paragraph: the chapter beats naive and off-the-shelf baselines by a measured margin (stock BiomedCLIP 3.40% i2t R@10 on this project's own protocol vs 17.14%), **does not approach the best specialised CXR retrieval architectures**, and is supporting evidence rather than a competitive result. No number is quoted for those architectures — none was read from a paper's own table, the same sourcing rule that keeps Janus-CXR uncited. **The audit also caught a live overclaim in the §Summary** — *"beats a retrieval-NN baseline on CheXbert F1 … not on a single lucky checkpoint"* — which 15B-4 overturned; it is now corrected in place, with the correction shown rather than quietly dropped, and §1.5 carries the seed band. `README.md` was checked and makes no retrieval or CheXbert claim at all. Original spec: Audit every retrieval claim in `analysis/h100_scaling_results.md` §2, `analysis/PHASE14_SUPERVISOR_REVIEW.md`, `README.md`, and `h100_scaling_state.json["primary_thesis"]` against the supervisor's note. §2 is already titled *"Retrieval (Supporting Chapter)"*, so this is expected to be small — but state explicitly, once, that the chapter **trails the best specialised retrieval architecture**, and make sure no summary line reads as a headline claim.
...
**Phase 15 (2026-09-13) — decide before the noted item:**
- 15A: if a published number cannot be sourced, does the row stay in the table marked `unverified` or come out entirely? Default: **stays, marked** — a visible gap is more honest than a silently shortened table.
- 15A-2 / 8F: the long-open `findings-only vs findings+impression` text-target question is now load-bearing for comparability. 15A reports the consequence; **changing the target is out of scope for Phase 15** (it invalidates every generation number in the project). Decide separately whether a findings-only arm is worth one 2h30m run purely for comparability with published work.
- 15C-3: if λ=0.1 and λ=0.5 straddle the optimum, is a third dose authorised, or does the better of the two proceed to 15C-4? Default: **proceed with the better of two** — user authorised one designed attempt, not a sweep.
- 15B-3: 3 seeds is the agreed scope. If the seed SD turns out to be large enough to swallow the 14A head-to-head result, is a 5th/6th seed authorised to tighten it? **Decide only if it happens** — do not pre-commit compute.
