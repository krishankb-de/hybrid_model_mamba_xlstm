...
- [x] **7D-setup** — **DONE (user, 2026-08-16).** `~/.netrc` created on the aisc box, `chmod 600`, credentials filled in.
- [ ] **7D-verify** — **Still open — the first attempt failed for an unrelated reason (see 7E): the login node itself refuses execution, before `.netrc`/auth is even reached.** Re-verify via Slurm, not a bare shell command:
      ```
      STAGE=meta OUT=/tmp/mimic_smoke_test sbatch scripts/build_mimic_cxr_local.sh
      # then: tail -f logs/mimic_build_<jobid>.log
      ```
      (`--account=default` is now baked into the script's `#SBATCH` header — see 7E — so nothing extra needs passing at submit time.)
      Confirm it downloads the ~150 MB of small files without a 401 (delete `/tmp/mimic_smoke_test` after: `rm -rf /tmp/mimic_smoke_test`). **Do not put the password in a script, a chat message, argv, or a SLURM log** — `build_mimic_cxr_local.py` uses Python `requests`, which reads `~/.netrc` automatically when no `auth=` is passed (`Session.trust_env`, the default), so the password never appears in a process list or a log line. This also means the earlier `wget --user --ask-password` approach would not have worked here regardless: `--ask-password` needs an interactive TTY, incompatible with an unattended `sbatch` job.
- [x] **7E** — **ANSWERED (2026-08-16), confirmed live + against official docs.** The login node **rejects every script execution outright** — `python build_mimic_cxr_local.py meta` on `lx01` was refused before making a single request: *"This command is not allowed on the login node!"* This is not the `bash <script>` restriction noted earlier; it is total. Per `docs.sc.hpi.de/cluster/Resources/{Login-Nodes,Data-Transfer,Partitions}` (fetched live): external downloads belong on **compute nodes via Slurm**, not a Run Node (`rx01`/`rx02` — 8h/4-core cap, explicitly *not* for data acquisition) and not the AISC GPU partitions (this job needs zero GPU). Use:
      - **`cpu-interactive`** (8h cap) — the `meta`/`manifest`/`pack` stages (all short, no network-duration risk for the latter two).
      - **`cpu-batch`** (7-day cap) — the long `fetch` stage. Resumable design (Phase 8A) means a timeout just needs a resubmit.
      - `scripts/build_mimic_cxr_local.sh` (NEW) wraps all four stages, `STAGE=` env-selected, submits to `cpu-batch`, **requests no GPU**. Tested: `test_build_mimic_cxr_local_slurm_wrapper_is_cpu_only_on_cpu_batch`.
      - **Correction (confirmed live, 2026-08-16):** this cluster requires an explicit `--account` on every `sbatch` — a plain `STAGE=meta ... sbatch scripts/build_mimic_cxr_local.sh` fails with `"No Slurm account specified"` before the job even queues. `--account=aisc` (every GPU script in this repo uses it) *submitted* but then sat `PD (QOSNotAllowed)` forever — the `aisc` account carries `QOS=aisc`, scoped to the AISC partitions (`aisc-batch`/`aisc-interactive`/`aisc-shortrun`), not the general `cpu-batch`/`cpu-interactive` partitions this script targets. `sacctmgr show user $USER withassoc format=User,Account,Partition,QOS` surfaced a second association: `account=default`, `QOS=normal` — the general-partition account. **Script default is now `--account=default`.**
