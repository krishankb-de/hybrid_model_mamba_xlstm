...
- [x] **9A-prep** — **Early-validation tooling built (2026-08-17), unblocking Arm 0 from waiting on the full ~10-day `fetch`.** The gap: `fetch` processes all 218,131 images in plain sequential chunk order, scattering the specific ~27,570 legacy-training studies across nearly all ~110 chunks — meaning they would not all be present until the run was nearly done, defeating the point of a cheap early sanity check. Fixed by adding a way to fetch *just* that subset, separately and in isolation:
      ...
      - **Run in a separate `OUT` directory** (e.g. `mimic_arm0_subset`), not inside the main `mimic_full` build — avoids any risk of two concurrent `fetch` invocations colliding on the same `_stage` scratch directory (each wipes `_stage` at its own chunk boundary; running two against one `OUT` would corrupt each other's in-flight chunks). Copy the already-built `manifest.parquet` from `mimic_full` into the new directory first — no need to re-run `meta`/`manifest`.
      **Sequence**: `sbatch scripts/dump_legacy_hashes.sh` → copy `manifest.parquet` into a fresh `OUT` → `STAGE=fetch OUT=.../mimic_arm0_subset STUDY_HASHES=legacy_training_hashes.txt sbatch scripts/build_mimic_cxr_local.sh` (small: ~27.5k images vs 218k, roughly 1/8th the wall time) → `STAGE=pack OUT=.../mimic_arm0_subset sbatch scripts/build_mimic_cxr_local.sh` → run the D1c recipe against it via the existing `DATASET_CONFIG=cxr_mimic_full` lever.
      ⚠️ **Expect under-matching**, same caveat as the 8D leakage guard: the legacy mirror's report text won't hash-match 1:1 against this pipeline's (official, vendored) parser output, so the resulting subset will likely be smaller than the full 27,570 — report the actual matched count, don't assume exact reproduction of the original set membership. The comparison is still meaningful as a *substantially-overlapping* reproduction check, not a bit-exact one.
