...
#### 15C — Auxiliary multi-label CheXpert loss 🔴 **NEGATIVE RESULT 2026-09-16 — the pre-registered stop rule fired.** Code shipped (15C-1/15C-2, harness 9/9); λ probe run (15C-3); **neither λ improved validate CheXbert-14-macro, so 15C-4's 3 seeds do NOT run** — one designed attempt, reported honestly (user decision 2026-09-13).
...
- [ ] **15C-5** — Write it up either way. **A negative here is worth more than 13F's was**, and for a stateable reason: 15B's seed band makes "smaller than seed variance" a real, quantified finding, where 13F could only report "non-monotonic across two doses" and could not separate effect from noise.
