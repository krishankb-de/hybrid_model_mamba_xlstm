...
>
> **Current phase: Phase 11 — Report-generation evaluation, full-data arm. Phase 8 (fetch+pack) and the full-data Phase 10E training run (job 2491338, 191,462 pairs) are DONE (2026-08-28). Full n=1433 eval (11C full, jobs 2491600/2491687) REVERSES the n=10 preliminary read: generator BEATS the retrieval-NN floor on all 3 metrics (rouge_l 0.2075 vs 0.1881; bleu_1 0.2706 vs 0.2605; bleu_4 0.0707 vs 0.0465). Caveat: 1055/1433 (73.6%) generated reports fall into one of 184 exact-duplicate template clusters — real signal but still heavily templated. 11B (CheXbert F1) harness BUILT 2026-08-28, LIVE-TESTED 2026-08-29 (job 2492037): first attempt guessed a wrong f1chexbert API and cleanly self-reported "skipped: ModuleNotFoundError" (package not yet installed) rather than crashing — since fixed against the real, PyPI-README-confirmed API (`F1CheXbert()(hyps=..., refs=...)`); the CSV ground-truth cross-check originally planned turned out not implementable against the public API and was dropped. NEXT ACTION: `git pull`, `pip install f1chexbert` on the cluster venv, then rerun `CHECKPOINT=./outputs/h100_report_gen_full/checkpoints/last.ckpt PARQUET=/sc/home/$USER/dataset/mimic_full/validate.parquet NUM_SAMPLES=1433 CHEXBERT=true sbatch scripts/inspect_report_generation_h100.sh`. Also open, independent: 11D (official test split), 11E (qualitative appendix).**
