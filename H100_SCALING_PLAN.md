...
#### 15A — Published field-baseline table ⏳ NOT STARTED (no compute)
...
- [ ] **15A-1** — Rows: R2Gen, CvT2DistilGPT2, RGRG, RA-RRG, Janus-CXR + this project's hybrid (13D), the matched Transformer, and the retrieval-NN floor. Metrics: ROUGE-L, BLEU-1, BLEU-4, CheXbert-14 micro/macro, CheXbert-5 micro/macro where published.
- [ ] **15A-2** — **The comparability columns are the point of the table, not decoration.** Record for every row: eval split (official MIMIC-CXR test vs custom), labeler (CheXbert vs CheXpert-labeler vs CheXpert-labeler-on-14), label set (14 vs 5), **target text**, and views (frontal-only vs all).
...
#### 15B — Statistical rigor for the generation table ⏳ NOT STARTED (the long pole)

- [ ] **15B-1** — **CIs on generator-vs-retrieval-floor.** This is the literal "Table 3 reports none" complaint. **Zero new code** — `scripts/bootstrap_compare.py` already takes any two hyp dumps. Blocker confirmed 2026-09-13: `results/retrieval_floor_test_split/` has `hyps.txt`/`refs.txt`/`chexbert_metrics.json` but **no `chexbert_labels.json`** (it was scored 2026-08-30, before `--dump-labels` existed), so re-score it once (~15 min) to get the label matrices, then bootstrap.
- [ ] **15B-2** — **Add a `SEED` lever.** `configs/config.yaml:13` hardcodes `seed: 42` and **no wrapper exposes it** — every report-gen run in this project's history is one seed. Add `SEED` to `scripts/train_report_generation_h100.sh` (passing `seed=${SEED}`) + a parity test asserting the lever exists, same shape as the existing `PREFIX_K`/`OVERSAMPLE_*` lever tests.
- [ ] **15B-3** — **Seeds 43 and 44 × both arms.** Seed 42 already exists for both (13D and the 14A Transformer), so this is **2 new hybrid runs (5h) + 2 new Transformer runs (1h20m)**, then generate + score each on the official test split. Matched seeds across arms **by design** — it makes 15B-4's comparison paired rather than unpaired.
