...
>
> **Current phase: Phase 11 — Report-generation evaluation, full-data arm. Phase 8 (fetch+pack) and the full-data Phase 10E training run (job 2491338, 191,462 pairs) are DONE (2026-08-28). Full n=1433 eval (11C full, jobs 2491600/2491687) REVERSES the n=10 preliminary read: generator BEATS the retrieval-NN floor on all 3 metrics (rouge_l 0.2075 vs 0.1881; bleu_1 0.2706 vs 0.2605; bleu_4 0.0707 vs 0.0465). Caveat: 1055/1433 (73.6%) generated reports fall into one of 184 exact-duplicate template clusters — real signal but still heavily templated. 11B (CheXbert F1) harness BUILT 2026-08-28, API FIXED + LIVE-TESTED 2026-08-29 against the real f1chexbert API; `f1chexbert` package then SUCCESSFULLY INSTALLED 2026-08-29 (`uv pip install f1chexbert` via `sbatch --wrap`, after bare `pip`/`python -m pip`/`srun --pty` all dead-ended — see 11B below). Full rerun at n=1433 with `CHEXBERT=true` (job 2493764) reproduced ROUGE-L/BLEU bit-for-bit but CheXbert itself STILL skipped: `OSError: couldn't connect to huggingface.co ... couldn't find them in the cached files` — root cause CONFIRMED as `HF_HUB_OFFLINE=1` (the SLURM wrappers' default, correct for the report-gen model but wrong for f1chexbert's first-ever checkpoint download) blocking a download the compute nodes otherwise have network for. NEXT ACTION: rerun with `HF_HUB_OFFLINE=0` overriding the wrapper default (cheap `NUM_SAMPLES=10` test first, then full `NUM_SAMPLES=1433`) — no code change needed. Also open, independent: 11D (official test split), 11E (qualitative appendix).**
