...
**Status: M0–V4 complete, 80/83 checkboxes.** Plan of record: [`MAMBA3_PLAN_V2.md`](MAMBA3_PLAN_V2.md);
...
### M7 screen result (12,000 steps, seed 42, paired)

| Arm | levers | val PPL | fit |
|---|---|---|---|
| A0 s42 / s1234 | Mamba-1, legacy scan | 19.387 / 18.933 | 2:36:50 |
| **A1** | **Mamba-1 + exact scan + Δ init + no Δ-norm** | **16.294** | 4:11:13 |
| **A2** | **SSD + 8× state** | **16.708** | **1:20:55** |
| A3 | + trapezoid | 16.719 | 1:28:00 |
| A4-lo / mid / hi | + RoPE, `theta_max` 0.002 / 0.02 / 0.2 | 16.431 / 16.534 / **16.199** | ~1:30 |
| A4 / A5 / A6 | + RoPE, `theta_max` 1.0 (81 turns) | 1166.7 | collapsed |

Seed-noise floor 0.454 PPL; pre-registered bar 0.642 PPL.

**The headline is the defect, not the architecture.** Fixing the divide-and-clamp and the Δ init on
the *existing* Mamba-1 block recovers **−3.09 PPL (−16.0%)**; SSD with 8× the state recovers −2.68.
`A1 − A2 = 0.414` — 64% of the bar, **not significant**. Both routes to a correct operator land in
the same place.

**What SSD buys is cost.** A2 trains **3.10× faster than A1** and 1.94× faster than A0. Correctness
...
| V4 | Writeup + cleanup (after V3) | ✅ 5/5 |
| V5 | Gated / optional (cluster access confirmed 2026-09-19; the remaining gates are scientific) | 🔄 2/4 |
