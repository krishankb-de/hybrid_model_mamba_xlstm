...
#SBATCH --job-name=biomedclip_kd_p15_pure
...
# Phase 11 ABLATION (PURE Phase-6e recipe on v2 backbone): freq_kd=false AND
# vit_unfreeze_blocks=0. Isolates the architectural refactor's contribution by
# stripping BOTH new contrastive deltas.
# Purpose: canonical v2 (freq_kd=false, vit_unfreeze=2) hit MIMIC 10.45% (target)
# but Indiana i2t 3.90% — 0.14pp under the 4.04% floor, converged. Hypothesis:
# the fine-tuned ViT specialises the image tower to MIMIC and hurts cross-domain
# Indiana; a FROZEN general BiomedCLIP ViT may transfer better. Compare MIMIC +
# Indiana R@10 vs the vit_unfreeze=2 run (outputs/biomedclip_kd_phase15_v2_nofreq):
#   - Indiana recovers >=4.04% & MIMIC stays high → drop ViT-unfreeze; clean
#     floor+target win attributable purely to the v2 backbone.
#   - Indiana stays ~3.9% → cross-domain gap is intrinsic v2 MIMIC-specialisation.
#
...
python scripts/train_contrastive.py \
  ...
  experiment_name=biomedclip_kd_phase15_v2_pure \
  output_dir=./outputs/biomedclip_kd_phase15_v2_pure \
  wandb.enabled=false

echo ""
echo "=== JOB END (BiomedCLIP-KD Phase 15 / refactor Phase 11 complete) ==="
echo "Best checkpoint: ./outputs/biomedclip_kd_phase15_v2_pure/checkpoints/"
date
