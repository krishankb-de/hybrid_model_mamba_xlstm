...
#SBATCH --job-name=biomedclip_kd_p15_nofreq
...
# Phase 11 ABLATION (freq_kd=false) — isolate freq-decoupled KD's effect.
# Identical to train_biomedclip_kd_phase15.sh EXCEPT distill.freq_kd=false.
# Purpose: Phase 11 v2 hit MIMIC i2t 10.94% (target) but Indiana 2.96% (< 4.04%
# floor) — cross-domain regression. ViT-unfreeze ruled out (helped Indiana
# slightly). This tests whether freq-KD's low-band emphasis overfit MIMIC's
# templated reports at the expense of Indiana generalization. Compare MIMIC +
# Indiana R@10 vs the freq_kd=true run (outputs/biomedclip_kd_phase15_v2).
#
...
python scripts/train_contrastive.py \
  ...
  experiment_name=biomedclip_kd_phase15_v2_nofreq \
  output_dir=./outputs/biomedclip_kd_phase15_v2_nofreq \
  wandb.enabled=false

echo ""
echo "=== JOB END (BiomedCLIP-KD Phase 15 / refactor Phase 11 complete) ==="
echo "Best checkpoint: ./outputs/biomedclip_kd_phase15_v2_nofreq/checkpoints/"
date
