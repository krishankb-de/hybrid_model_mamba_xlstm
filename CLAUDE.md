...
Before any work, read `HYBRID_ARCH_REFACTOR_PLAN.md` (committed) and `hybrid_arch_refactor_state.json` (committed; allowlisted in `.gitignore`) at repo root. They are the resumable plan-of-record for the pipeline (architectural refactor addressing PDF gaps 1–6: mLSTM stabilization, HybridNorm, layer pattern, cu_seqlens doc-boundary resets, WSD scheduler, freq-decoupled KD — supersedes the BiomedCLIP-text-KD recipe-only path which plateaued at MIMIC R@10 ~8–10%). Full approved plan: `/Users/krish/.claude/plans/refer-to-the-plan-mellow-peacock.md`. Resume at `hybrid_arch_refactor_state.json["current_phase"]`; checkboxes in `HYBRID_ARCH_REFACTOR_PLAN.md` are ground truth for completed work. After every meaningful state change (run submitted, run finished, eval completed), update both the relevant checkbox AND `hybrid_arch_refactor_state.json["last_updated"]` + append a one-line note to `hybrid_arch_refactor_state.json["notes"]`. If `hybrid_arch_refactor_state.json` is missing, regenerate from `HYBRID_ARCH_REFACTOR_PLAN.md` checkbox state. `BIOMEDCLIP_KD_PLAN.md` + `biomedclip_kd_state.json` and `JOINT_TRAINING_PLAN.md` + `joint_training_state.json` are kept on disk as deprecated historical record only — do NOT resume against them.
